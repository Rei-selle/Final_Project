...
if ($result->num_rows > 0) {
            ...
            if ($last_order_date !== null) {
                ...
                <p class='subtotal'>Subtotal: ₱" . number_format($order_subtotal, 0) . "</p>
                    <form>
                        ...
                        <button type='button' class='btn processing'>Ongoing</button>
                        <input type='hidden' name='order_date' value='{$last_order_date}'>
                        <button type='button' class='btn processing'>Delivered</button>
                        <input type='hidden' name='order_date' value='{$last_order_date}'>
                        <button type='button' class='btn processing'>Complete</button>
                    </form>
                 </div>";
    ...
    <p class='subtotal'>Subtotal: ₱" . number_format($order_subtotal, 0) . "</p>
        <form>
            <input type='hidden' name='order_date' value='{$last_order_date}'>
            <button type='button' class='btn processing ongoing'>Ongoing</button>
            <input type='hidden' name='order_date' value='{$last_order_date}'>
            <button type='button' class='btn processing delivered'>Delivered</button>
            <input type='hidden' name='order_date' value='{$last_order_date}'>
            <button type='button' class='btn processing complete'>Complete</button>
        </form>
      </div>";
} else {
...
window.onload = function() {
    ...
    processingButtons.forEach(button => {
        button.addEventListener('click', function(event) {
            ...
            console.log('Button clicked: ', this.textContent.trim()); // Debugging line
